...
# --- Configuration ---
# Read from environment or fall back to localhost
...
main() {
    ...
    response=$(curl -s -w \"%{http_code}\" $CURL_OPTS -o response.json "$BASE_URL/healthz")
    ...
    response=$(curl -s -w \"%{http_code}\" $CURL_OPTS -o response.json -X POST "$BASE_URL/auth/register" \
    ...
    response=$(curl -s -w \"%{http_code}\" $CURL_OPTS -o response.json -X POST "$BASE_URL/auth/login" \
        -H "Content-Type: application/json" \
    ...
    response=$(curl -s -w \"%{http_code}\" $CURL_OPTS -o response.json -X GET "$BASE_URL/users/profile" \
    ...
    response=$(curl -s -w \"%{http_code}\" $CURL_OPTS -o response.json -X POST "$BASE_URL/auth/refresh" \
        -H "Content-Type: application/json" \
    ...
    response=$(curl -s -w \"%{http_code}\" $CURL_OPTS -o response.json -X POST "$BASE_URL/auth/logout" \
        -H "Content-Type: application/json" \
    ...
    # --- Internal API Tests (Skipped - Only accessible within cluster) ---
    # These tests are commented out because internal endpoints are not exposed through the ALB/Ingress
    # They should only be tested from within the Kubernetes cluster (e.g., from another service pod)
    
    # print_header "12. Internal - Verify Token (Success)"
    # response=$(curl -s -w \"%{http_code}\" $CURL_OPTS -o response.json -X POST "$BASE_URL/internal/auth/verify" \
    ...
    # response=$(curl -s -w \"%{http_code}\" $CURL_OPTS -o response.json -X GET "$BASE_URL/internal/users/$USER_ID" \
    #     -H "Authorization: ApiKey $INTERNAL_API_KEY")
    # check_status 200 "$response" "Internal Get User"
    # cat response.json

    echo -e "\n\033[0;32m==================================================\033[0m"
    echo -e "\033[0;32m  All External API Tests Passed Successfully!\033[0m"
    echo -e "\033[0;32m  (Internal API tests skipped - cluster-only access)\033[0m"
